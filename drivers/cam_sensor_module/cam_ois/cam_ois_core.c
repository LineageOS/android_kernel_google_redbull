	...
	stReCalib *cal_result)
{
	...
	rc = GyroReCalibrate(&o_ctrl->io_master_info, cal_result);
	if (rc != 0)
		CAM_ERR(CAM_OIS,
			"[OISCali] ReCalib FAIL, rc = %d", rc);
	else {
		rc = WrGyroOffsetData(&o_ctrl->io_master_info, cal_result);
		if (rc != 0)
			CAM_ERR(CAM_OIS,
...
static void cam_ois_read_work(struct kthread_work *work)
{
	...
	if (ois_debug) {
		...
		CAM_INFO(CAM_OIS,
			"[0xE003] %s: buf[0-1]=%02x%02x, buf[2-3]=%02x%02x, "
				"buf[4-5]=%02x%02x, buf[6-7]=%02x%02x",
			ois_timer_in->o_ctrl->ois_name, buf[0], buf[1], buf[2], buf[3],
		...
		rc = camera_io_dev_read_seq(
			...
			CAMERA_SENSOR_I2C_TYPE_DWORD, 8);
		CAM_INFO(CAM_OIS,
			"[0xE005] %s: buf[0-1]=%02x%02x, buf[2-3]=%02x%02x, "
				"buf[4-5]=%02x%02x, buf[6-7]=%02x%02x",
			ois_timer_in->o_ctrl->ois_name, buf[0], buf[1], buf[2], buf[3],
			buf[4], buf[5], buf[6], buf[7]);
	}
	...
	if (ois_debug) {
		CAM_INFO(CAM_OIS,
			"[0xE001] %s: buf[0-1]=%02x%02x, buf[2-3]=%02x%02x, "
				"buf[4-5]=%02x%02x",
			ois_timer_in->o_ctrl->ois_name, buf[0], buf[1], buf[2], buf[3],
			buf[4], buf[5]);
	}
	...
	ois_shift_data.af_lop1 =
		(int32_t)(((uint32_t)buf[6] << 24) | ((uint32_t)buf[7] << 16) |
